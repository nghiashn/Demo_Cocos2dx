...
GameLayer::GameLayer()
{
    ...
    //Texture2D* texture = TextureCache::sharedTextureCache()->textureForKey("pd_sprites.png");
    //Texture2D* texture = TextureCache::getTextureForKey
...
void GameLayer::initTileMap()
{
    ...
    Object* itemLayer;
    ARRAY_FOREACH(mTileMap->getChildren(), itemLayer)
    {
...
{
    mHero->setPosition(Point(mHero->mCenterToSide, 50));
...
void GameLayer::updatePosition(float dt)
{
   /* float posX = GetMIN(mTileMap->getMapSize().width * mTileMap->getTileSize().width - mHero->mCenterToSide,
    ...
    mHero->setPosition(Point(posX, posY));

    Object* item;
    ARRAY_FOREACH(mRobots, item) {
...
//{
//    int robotCount = 50;
//    this->mRobots = new Vector(robotCount);
//
...
void GameLayer::reorderActors()
{
    ...
    ARRAY_FOREACH(this->getChildren(), item) {
